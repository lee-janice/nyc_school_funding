...
SELECT 
    ...
    -- sum of flags 
    SUM(BALANCE_WY_DIFF_FLAG) + SUM(BALANCE_XY_DIFF_FLAG) + SUM(list_sum(list_value(*COLUMNS('pta.*flag')))) AS N_ANOMALIES,
    SUM(BALANCE_WY_DIFF_FLAG) AS N_BALANCE_WY_DIFF_FLAG, 
    SUM(BALANCE_XY_DIFF_FLAG) AS N_BALANCE_XY_DIFF_FLAG, 
